...
namespace NodeCanvas.Tasks.Actions
{
    ...
    public class MoveTowards : ActionTask<Transform>
    {
        ...
        protected override void OnUpdate() {
            ...
            agent.position = Vector3.MoveTowards(agent.position, new Vector3(target.value.transform.position.x, 0f, target.value.transform.position.z), speed.value * Time.deltaTime);
            if ( !waitActionFinish ) {
                EndAction();
            }
        }
